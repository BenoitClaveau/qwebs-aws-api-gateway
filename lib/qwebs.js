...
class QwebsAwsQwebs extends Qwebs {
    ...
    static create(options) {
        return Singleton.init(() => new QwebsAwsQwebs(options));
    }
    ...
    constructor(options) {
        ...
        if (!this.config.http) throw new DataError({ message: "Http section must be defined in config file." });
        if (this.config.http) this.inject("$http", "qwebs-http");   //avoid to add qwebs-http in config.json
    };
};
